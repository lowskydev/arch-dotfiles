...
| `yazi`                        | Terminal file manager (Rust, async, Kitty graphics protocol previews) |
| `thunar`                      | GUI file manager (XFCE, lightweight, Wayland support)                 |
...
| `fd`                | Fast file finder (required by yazi)                      |
| `poppler`           | PDF rendering (required by yazi PDF previews)            |
| `ffmpeg`            | Video/audio processing (required by yazi video previews) |
| `7zip`              | Archive handling (required by yazi archive previews)     |
...
Some apps does not reliably read live gsettings/env theme values.
...
Create `~/.config/gtk-4.0/settings.ini` (for GTK4 apps like Nautilus, if ever installed):
